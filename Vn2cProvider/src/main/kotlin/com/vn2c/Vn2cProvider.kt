...
class Vn2cProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        matches.forEach { matchResult ->
            ...
            // Gửi link tìm được cho Cloudstream phát (Truyền giá trị trực tiếp)
            // Gửi link tìm được cho Cloudstream phát (Sử dụng cú pháp mới của Cloudstream)
            // Gửi link tìm được cho Cloudstream phát (Cloudstream mới sẽ tự nhận diện m3u8)
            callback(
                newExtractorLink(
                    source = name,
                    name = name,
